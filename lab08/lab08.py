...
def add_trees(t1, t2):
    ...
    """
    "*** YOUR CODE HERE ***"
    lst = list(zip(t1.branches, t2.branches))
    lst = [add_trees(b[0], b[1]) for b in lst]
    if len(t1.branches) > len(lst):
        lst += t1.branches[len(lst):]
    else:
        lst += t2.branches[len(lst):]
    return Tree(t1.root + t2.root, lst)
