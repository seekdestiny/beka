...
class Frame:
    ...
    def lookup(self, symbol):
        ...
        # BEGIN PROBLEM 3
        "*** REPLACE THIS LINE ***"
        if self is not None and symbol in self.bindings:
...
def make_let_frame(bindings, env):
    ...
    # BEGIN PROBLEM 15
    "*** REPLACE THIS LINE ***"
    formals_lst, values_lst = nil, nil
    for i in range(len(bindings)):
        check_form(bindings.first, 2, 2)
        formals_lst = Pair(bindings.first.first, formals_lst)
        values_lst = Pair(scheme_eval(bindings.first.second.first, env), values_lst)
