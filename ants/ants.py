...
class Ant(Insect):
    ...
    def can_contain(self, ant):
        return self.container == True and ant.container == False and self.ant == None
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, hive):
        ...
        place = self.place
        distance = 0
        while place is not None and place is not hive:
...
class ScubaThrower(ThrowerAnt):
    """A ThrowerAnt that is water safe."""
    name = 'Scuba'
    food_cost = 6
    watersafe = True
    implemented = True   # Change to True to view in the GUI
...
class QueenAnt(ScubaThrower):  # You should change this line
    ...
    def __init__(self):
        ...
        "*** REPLACE THIS LINE ***"
        Ant.__init__(self, armor=1)
        self.imposter = False if QueenAnt.queen_count == 0 else True
        ...
        if self.imposter == True:
            ...
            while place is not None:
                if place.ant is not None:
                    if hasattr(place.ant, 'damage') and place.ant not in self.doubled:
                        place.ant.damage = place.ant.damage * 2
                        self.doubled.append(place.ant)
                    if hasattr(place.ant, 'container') and place.ant.container:
                        if place.ant.ant is not None and hasattr(place.ant.ant, 'damage') \
                            and place.ant.ant not in self.doubled:
